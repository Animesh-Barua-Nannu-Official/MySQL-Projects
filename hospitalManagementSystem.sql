...
apptime time default "10:00:00",
userStatus int not null,
docStatus int not null
);
drop table appointmenttb;
select * from appointmenttb;
insert into appointmenttb(p_id, id, first_name, last_name, gender, 
email, contact, doctor, docFees, appdate, apptime, userStatus, docStatus) values
...
(4, 10, 'Kishan', 'Lal', 'Male', 'kishansmart0@gmail.com', '8838489464', 'Ganesh', 550, '2020-01-01', '14:00:00', 1, 0),
...
insert into doctb(username, password, email, spec, docFrees) values 
...
foreign key (p_id) references appointmenttb(p_id)
on delete cascade
on update cascade
);

insert into patreg(p_id, fname, lname, gender, email, contact, password, cpassword) values
...
select * from appointmenttb as ap
left join patreg as p
on ap.p_id = p.p_id
union
select * from appointmenttb as ap
right join patreg as p
on ap.p_id = p.p_id ;